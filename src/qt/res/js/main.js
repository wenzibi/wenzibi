    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                <label for="pay_to[count]" class="recipient">发送到:</label> \
                <input id="pay_to[count]" class="pay_to input_box" data-title="交易发送地址  (例如. SXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" placeholder="数据发送地址 (例如. SXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
                <a id="address_lookup[count]" class="button is-inverse has-fixed-icon" data-title="从通讯录中添加" style="margin-right:10px; margin-left:10px; height:43px; width:43px;" href="#address-lookup-modal" onclick="returnto=\'pay_to[count]\,label[count]\';prepAddressLookup(false); " ><i class="fa fa-book"></i></a> \
                <a class="button is-inverse has-fixed-icon" data-title="从剪切板中粘贴" style="margin-right:10px; height:43px; width:43px;" onclick="paste(\'#pay_to[count]\')"><i class="fa fa-files-o"></i></a> \
                <a class="button is-inverse has-fixed-icon" data-title="移出该收件人" style="height:43px; width:43px;" onclick="if($(\'div.recipient\').length == 1) clearRecipients(); else {var recipient=$(\'#recipient[count]\');if(recipient.next(\'hr\').remove().length==0)recipient.prev(\'hr\').remove();$(\'#recipient[count]\').remove();resizeFooter();}"><i class="fa fa-times"></i></a> \
            </div> \
            <div class="flex-right"> \
                <label for="label[count]" class="recipient">标签:</label> \
                <input id="label[count]" class="label input_box" data-title="在地址簿中为该地址添加一个标签" placeholder="在地址簿中为该地址添加一个标签" maxlength="128"/> \
            </div> \
            <div class="flex-right"> \
                <label for="narration[count]" class="recipient">留言:</label> \
                <input id="narration[count]" class="narration input_box" data-title="为该交易添加留言(最大24个字符)" placeholder="为该交易添加留言(最大24个字符)" maxlength="24" /> \
            </div> \
            <div class="flex-left"> \
                <label for="amount[count]" class="recipient">数量:</label> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="wzcoin"                    ' + (unit.type == 0 ? "selected" : "") + '>WZC</option> \
                    <option value="1" title="Milli-wzcoin (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mWZC</option> \
                    <option value="2" title="Micro-wzcoin (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;WZC</option> \
                    <option value="3" title="sWZC (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>sWZC</option> \
                </select> \
            </div> \
...
function suggestRingSize()
{
    ...
    for(var i=0;i<recipients;i++)
    {
        ...
        {
            invalid(el);
            el.parent().after("<div name='err"+el.attr('id')+"' class='warning'>余额不足.<br><br>Only <b>"
                     +maxsize+"</b> 匿名输出存在于总额中的值: <b>" + unit.format(output*(test/10), $("#unit"+i)) + "</b></div>");
...
function newConversation() {
    ...
    $("#message-text").focus();
    $(".contact-discussion ul").html("<li id='remove-on-send'>开始谈话 "+$("#new-contact-address").val()+" - "+$("#new-contact-name").val()+"</li>");
...
function deleteMessages(key, messageid) {
    ...
    if(!confirm("你确定删除 " + (messageid == undefined ? '这些消息?' : '消息?')))
        return false;
