// Copyright (c) 2014 The wzcoin developers
...
void ShadowGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About wzcoin"), this);
    aboutAction->setToolTip(tr("Show information about wzcoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for wzcoin"));
...
void ShadowGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("wzcoin") + " - " + tr("Wallet") + ", " + sMode);
        };
        ...
        if (clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("wzcoin client") + QString(" ") + tr("[testnet]"));
...
void ShadowGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to wzcoin network", "", count));
}
...
void ShadowGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid wzcoin address or malformed URI parameters."));
    }
