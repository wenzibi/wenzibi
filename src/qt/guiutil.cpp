...
{
    // Convert wzcoin:// to wzcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("wzcoin://"))
    {
        uri.replace(0, 12, "wzcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "wzcoin.lnk";
}
...
{
    return GetAutostartDir() / "wzcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "[Desktop Entry]\n" \
                   ...
                   << "MimeType=x-scheme-handler/wzcoin;\n";
        optionFile.close();
    }
    return true;
}
